...
export default function Checkout() {
  ...
  const handleWaitlistSubmit = async (e: React.FormEvent) => {
    ...
    if (!emailRegex.test(waitlistEmail.trim())) {
      toast({ title: "אנא הכניסו כתובת אימייל תקינה", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      toast({ title: "שגיאה", description: err.message || "אנא נסו שנית", variant: "destructive" });
    } finally {
      ...
      <div className="relative max-w-4xl mx-auto px-4 md:px-8 py-12" dir="rtl">
        ...
        <div className="">
          ...
          <div className="flex flex-col items-center gap-10">
            ...
            <form onSubmit={handleSubmit} className="w-full space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-[#1B1919] text-lg font-normal mb-3 text-right">שם בעל הכרטיס</label>
                  <input type="text" value={formData.cardHolderName} onChange={(e) => handleInputChange("cardHolderName", e.target.value)} placeholder="הכניסו שם כאן" className="w-full h-14 px-5 rounded-lg border border-[#4C7EFB] bg-[#F5F7FC] text-[#3C4382] text-right text-lg focus:outline-none focus:ring-2 focus:ring-[#4C7EFB]" />
                </div>
                <div>
                  <label className="block text-[#1B1919] text-lg font-normal mb-3 text-right">תעודת זהות מחזיק כרטיס</label>
                  <input type="text" value={formData.idNumber} onChange={(e) => handleInputChange("idNumber", e.target.value)} placeholder="הכניסו מספר תעודת זהות" className="w-full h-14 px-5 rounded-lg border border-[#4C7EFB] bg-[#F5F7FC] text-[#3C4382] text-right text-lg focus:outline-none focus:ring-2 focus:ring-[#4C7EFB]" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-[#1B1919] text-lg font-normal mb-3 text-right">תאריך תוקף הכרטיס</label>
                  <div className="flex gap-4" dir="rtl">
                    <div className="relative flex-1">
                      <select value={formData.expiryYear} onChange={(e) => handleInputChange("expiryYear", e.target.value)} className="w-full h-14 px-5 pr-10 rounded-lg border border-[#4C7EFB] bg-[#F5F7FC] text-[#3C4382] text-center text-lg focus:outline-none focus:ring-2 focus:ring-[#4C7EFB] appearance-none cursor-pointer">
                    ...
                    <div className="relative flex-1">
                      <select value={formData.expiryMonth} onChange={(e) => handleInputChange("expiryMonth", e.target.value)} className="w-full h-14 px-5 pr-10 rounded-lg border border-[#4C7EFB] bg-[#F5F7FC] text-[#3C4382] text-center text-lg focus:outline-none focus:ring-2 focus:ring-[#4C7EFB] appearance-none cursor-pointer">
                ...
                <div>
                  <label className="block text-[#1B1919] text-lg font-normal mb-3 text-right">CVV (3 ספרות בגב הכרטיס)</label>
                  <input type="text" value={formData.cvv} onChange={(e) => handleInputChange("cvv", e.target.value)} placeholder="XXX" maxLength={3} className="w-full h-14 px-5 rounded-lg border border-[#4C7EFB] bg-[#F5F7FC] text-[#3C4382] text-right text-lg tracking-widest focus:outline-none focus:ring-2 focus:ring-[#4C7EFB]" />
                </div>
              </div>
            </form>
          </div>
          ...
          <div className="flex flex-col items-center gap-10 mt-16">
            <h3 className="text-[#486284] text-2xl md:text-3xl font-bold text-center">אופציות נוספות לתשלום:</h3>
            <div className="flex flex-wrap items-center justify-center gap-4" dir="rtl">
              <button type="button" onClick={() => handleAlternativePayment("Google Pay")} className="flex items-center justify-center gap-2 px-5 py-3 min-w-[160px] rounded-xl border-2 border-[#DBE3F3] bg-white hover:border-[#4C7EFB] hover:shadow-lg hover:-translate-y-0.5 transition-all">
              ...
              <button type="button" onClick={() => handleAlternativePayment("Apple Pay")} className="flex items-center justify-center gap-2 px-5 py-3 min-w-[160px] rounded-xl border-2 border-[#DBE3F3] bg-white hover:border-[#4C7EFB] hover:shadow-lg hover:-translate-y-0.5 transition-all">
                <span className="text-[#1B1919] text-base">שלמו עם -</span>
              ...
              <button type="button" onClick={() => handleAlternativePayment("Bit")} className="flex items-center justify-center gap-2 px-5 py-3 min-w-[160px] rounded-xl border-2 border-[#DBE3F3] bg-white hover:border-[#4C7EFB] hover:shadow-lg hover:-translate-y-0.5 transition-all">
                <span className="text-[#1B1919] text-base">שלמו עם -</span>
          ...
          <div className="flex justify-center mt-12">
            <button type="submit" onClick={handleSubmit} disabled={isSubmitting} className="px-12 py-3 rounded-full bg-[#4C7EFB] text-white text-lg font-bold shadow-[6px_6px_0_0_rgba(0,0,0,0.1)] hover:shadow-[8px_8px_0_0_rgba(0,0,0,0.15)] hover:-translate-y-0.5 transition-all disabled:opacity-50 disabled:cursor-not-allowed">

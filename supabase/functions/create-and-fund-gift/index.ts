...
const userDataSchema = z.object({
  ...
  phone: z.string().transform(val => val.replace(/[\s\-\(\)]/g, '')).pipe(z.string().min(9).max(15).regex(/^(\+9725\d{8}|05\d{8})$/, 'Phone must be a valid Israeli mobile format (+9725XXXXXXXX or 05XXXXXXXX)')),
